...
export function Gallery() {
  ...
  return (
    <>
      <h2 className="text-2xl lg:text-3xl font-bold text-center font-robotomono text-greendark mb-4 mt-10 lg:mt-20">
        Galeria
      </h2>
      <div className="flex flex-wrap justify-center gap-4 lg:gap-5 p-4 mx-4 lg:mr-60 lg:ml-60">
        {images.map((media) => (
          <a
            key={media.id}
            href={media.permalink}
          ...
          >
            {media.media_type === 'VIDEO' ? (
              <video
                controls
                className="w-full lg:w-[350px] object-cover rounded-lg"
                src={media.media_url}
              />
            ) : (
              <img
                src={media.media_url}
                alt="Instagram Post"
                className="w-full lg:w-[350px] object-cover rounded-lg cursor-pointer"
              />
            )}
          </a>
        ))}
